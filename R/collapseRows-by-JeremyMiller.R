...
#     outputs a single number. This function will return(0) or crash otherwise.
        ...
        recMethods = c("function","MaxMean","maxRowVariance","MinMean","absMinMean","absMaxMean");
	...
	if (is.na(imethod)) {
	   ...
	   printFlush(" *MaxMean*, *MinMean*, *absMaxMean*, *absMinMean*, or *function* for a user-defined function.")
           return(0)
	}
	...
	if (!is.function(method)) method = get(method, mode = "function")
		
## Format the variables for use by this function
	rowID[is.na(rowID)] = rowGroup[is.na(rowID)] # Use gene if probe is missing
