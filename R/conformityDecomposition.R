conformityDecomposition = function (adj, Cl = NULL) 
{
   ...
   if ( is.null(Cl) )  {
    ...
    Factorizability = 1 - sum((adj - outer(Conformity, Conformity) + 
        diag(Conformity^2))^2)/sum(adj^2)}
    output = list(Conformity=data.frame( Conformity),  Factorizability = Factorizability, IntermodularAdjacency=NA)
   } 
...
numeratorFactorizability=0
   for (i in 1:length(Cl.level) ) {
    ...
    if (sum(restclass)==1) { A.help=0; CF.help =0 }
if (sum(restclass)==2) {
...
if (length(CF.help)>1) {numeratorFactorizability= numeratorFactorizability+sum( A.help-outer(CF.help,CF.help)+diag(CF.help^2))^2}
 ...
 if (  length(listConformity[[i]])==1 |  length(listConformity[[j]])==1    )   {
  numeratorFactorizability=  numeratorFactorizability+ sum( A.inter- IntermodularAdjacency[i,j]* listConformity[[i]] * listConformity[[j]]  )
  } else {
  numeratorFactorizability=  numeratorFactorizability+ sum( A.inter- IntermodularAdjacency[i,j]*outer( listConformity[[i]] , listConformity[[j]])) }
   } # end of for (i in
   } # end of for (j in
 Factorizability=  1-numeratorFactorizability/ sum(adj^2,na.rm=T)  
   } # end of if else statement
   
    output = list(Conformity=listConformity,  IntermodularAdjacency= IntermodularAdjacency, Factorizability = Factorizability,  IntramodularFactorizability=  IntramodularFactorizability, Cl.level =Cl.level)
